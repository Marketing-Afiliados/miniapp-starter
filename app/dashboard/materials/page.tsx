  ...
  return (
    <div>
      ...
      <section className="mt-6 grid gap-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(320px,.6fr)]">
        <div className="grid gap-3 sm:grid-cols-2">
          {materials.length ? materials.map((material) => (
            <article className={`rounded-2xl border bg-white p-5 shadow-sm ${material.active ? "border-slate-200" : "border-slate-100 opacity-70"}`} key={material.id}>
              <div className="flex justify-between gap-3"><div><h2 className="font-semibold">{material.name}</h2><p className="mt-1 text-xs text-slate-400">{material.active ? "Activo" : "Inactivo"} · por {material.unit}</p></div><span className="text-sm font-semibold text-violet-700">{formatCurrency(material.unit_cost_cents)}</span></div>
              <details className="mt-4"><summary className="cursor-pointer list-none text-sm font-semibold text-violet-600">Editar</summary><div className="mt-4 border-t pt-4"><MaterialForm material={material} /></div></details>
        ...
        <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:sticky lg:top-8" id="nuevo"><h2 className="text-lg font-semibold">Nuevo material</h2><div className="mt-5"><MaterialForm /></div></aside>
      </section>
    </div>
  );
}
