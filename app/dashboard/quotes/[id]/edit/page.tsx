...
export default async function EditQuotePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div><PageHeader eyebrow={quote.quote_number} title="Editar cotización" description="Actualiza los datos y vuelve a calcular en tiempo real." /><div className="mt-8"><QuoteEditor catalogCategories={catalog.categories} catalogItems={catalog.catalogItems} currency={quote.currency} customers={customers ?? []} defaultMargin={business.default_margin_percentage} defaultTerms={business.default_terms ?? ""} initial={initial} materials={materials ?? []} preferredCategoryIds={catalog.preferredCategoryIds} quoteId={id} services={services ?? []} /></div></div>;
}
