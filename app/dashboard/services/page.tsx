  ...
  return (
    <div>
      ...
      <section className="mt-6 grid gap-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(320px,.6fr)]">
        <div className="grid gap-3 sm:grid-cols-2">
          {services.length ? services.map((service) => (
            <article className={`rounded-2xl border bg-white p-5 shadow-sm ${service.active ? "border-slate-200" : "border-slate-100 opacity-70"}`} key={service.id}>
              ...
              <details className="mt-4"><summary className="cursor-pointer list-none text-sm font-semibold text-violet-600">Editar</summary><div className="mt-4 border-t pt-4"><ServiceForm currency={currency} service={service} /></div></details>
        ...
        <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:sticky lg:top-8" id="nuevo"><h2 className="text-lg font-semibold">Nuevo servicio</h2><div className="mt-5"><ServiceForm currency={currency} /></div></aside>
      </section>
    </div>
  );
}
